...
## Current State
- Scheduling and recovery contract is defined with account-level heartbeat semantics.
- Runtime registration and full execution path are pending implementation.

## Target State
- Deterministic per-account heartbeat execution with restart-safe recovery.
- Full deduplication and diagnostics visibility for heartbeat operations.

## Gap Notes
- Job registration, cursor updates, and degraded fallback signaling need runtime completion.
...
## Acceptance Criteria
1. Active accounts register deterministic heartbeat jobs.
2. Restart recovery replays missed and interrupted jobs without duplication.
3. Heartbeat health and failures are visible in diagnostics.

## Immediate Roadmap
1. Implement per-account scheduler registration and cursor-safe fetch pipeline.
2. Integrate catch-up semantics with cron recovery flow.
3. Align execution gates with feature plan item FP-011.
