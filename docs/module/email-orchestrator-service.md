...
## Current State
- Contract-level orchestration is fully specified.
- Runtime service wiring for full collaborative drafting lifecycle is still pending.

## Target State
- Provide end-to-end orchestration from selected action items to approved saved drafts.
- Preserve deterministic agent assignment, attribution, and Director review gating.

## Gap Notes
- Shared draft lifecycle, contribution merge mechanics, and complete IPC flow need implementation completion.
...
## Acceptance Criteria
1. Action items route to correct target agents based on contract rules.
2. Draft contributions merge deterministically with attribution.
3. Director approval is required before final persist.

## Immediate Roadmap
1. Implement shared draft orchestration service and persistence contracts.
2. Implement contribution merge pipeline and review checkpoints.
3. Integrate with feature plan items FP-010 and FP-011.
