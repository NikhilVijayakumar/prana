...
import { dataFilterService, RegistrySyncSnapshot } from '../operations/dataFilterService';
import { registryRuntimeStoreService } from '../registry/registryRuntimeStoreService';
import { syncStoreService } from './syncStoreService';
import { vaultService } from '../vault/vaultService';
import { recoveryOrchestratorService } from '../operations/recoveryOrchestratorService';
import { getRuntimeBootstrapConfig } from '../../common/config/runtimeConfigService';
import { getAppDataRoot } from '../governance/governanceRepoService';
...
export interface ActiveClientLock {
  machineId: string;
  hostName: string;
  userName: string;
  startedAt: string;
...
/**
 * Factory function to create a sync provider.
 * This is transitional - will be fully stateless in v3.
 */
export const createSyncProvider = () => {
  ...
  return {
    ...
    __resetForTesting(): void {
      ...
      lastIntegrityValid = null;
      lastIntegrityIssues = [];
      lastStartupInstallMode = null;
      lastStartupPullStatus = null;
      lastStartupMergeStatus = null;
      lastStartupIntegrityStatus = null;
      lastStartupSyncMessage = null;
      syncOperationQueue = Promise.resolve();
    },
  };
};

// Backward compatibility - creates a default instance
export const syncProviderService = createSyncProvider();
