...
import { vaidyarService, VaidyarHealthEvent } from '../governance/vaidyarService';
...
/**
 * Factory function to create a notification centre.
 * Eliminates module-level state for listeners, appId, and telemetry.
 */
...
const defaultNotificationCentre = createNotificationCentre();

export const notificationCentreService = defaultNotificationCentre;

// Convenience functions that delegate to the default instance
export async function initialize(currentAppId: string): Promise<void> {
  return defaultNotificationCentre.initialize(currentAppId);
}

export function subscribe(listener: NotificationListener): () => void {
  return defaultNotificationCentre.subscribe(listener);
}
...
export async function emit(event: {
  ...
  return defaultNotificationCentre.emit(event);
}
